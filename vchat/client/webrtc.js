...
function start() {
  ...
  document.getElementById('localVideoContainer').appendChild(makeMuteButton("Local Mute"));
...
function gotMessageFromServer(message) {
  ...
  // Yell murder if we see messages from another room!
  if (peerRoom != roomHash) {
    console.error('bloody murder! we are in room ' + roomHash + ' but got messages from room ' + peerRoom);
  }
...
function gotIceCandidate(event, peerUuid) {
  if (event.candidate != null) {
    serverConnection.send(JSON.stringify({ 'ice': event.candidate, 'uuid': localUuid, 'room': roomHash, 'dest': peerUuid }));
  }
}
...
function createdDescription(description, peerUuid) {
  ...
  peerConnections[peerUuid].pc.setLocalDescription(description).then(function () {
    serverConnection.send(JSON.stringify({ 'sdp': peerConnections[peerUuid].pc.localDescription, 'uuid': localUuid, 'room': roomHash, 'dest': peerUuid }));
  }).catch(errorHandler);
}
...
function gotRemoteStream(event, peerUuid) {
  ...
  vidContainer.appendChild(makeMuteButton("test (dont press)"));
...
function makeMuteButton(label) {
  var button = document.createElement('BUTTON');
  button.appendChild(document.createTextNode(label));
  button.setAttribute('class', 'muteButton');
  button.onclick = localMute();
  return button;
}
...
function localMute() {
  ...
  document.getElementById('localVideo').srcObject.getTracks()[0].enabled = localMuted;
}
